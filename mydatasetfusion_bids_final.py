# -*- coding: utf-8 -*-
"""mydatasetfusion_bids_final.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1TgWv2LDu_l-Q5iYHzzFpyPkvev10Cllw
"""
...
def plot_radar(personality_scores, labels=["O", "C", "E", "A", "N"]):
...
IEMOCAP_PATH = "/content/drive/MyDrive/SpeechMajorProject_b"
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        pred = model(sample_audio.unsqueeze(0).cuda(), sample_text.unsqueeze(0).cuda()).squeeze().cpu()
...
def evaluate_fusion_model(test_dataset, model, save_reports=False, report_dir="fusion_reports"):
    ...
    for idx in tqdm(range(len(test_dataset))):
        ...
        paths.append(f"sample_{idx}")

        if save_reports:
            fig = plot_radar(torch.tensor(pred), show=False)
            generate_pdf(pred, fig, output_path=os.path.join(report_dir, f"{paths[-1]}_report.pdf"))
    ...
    for i, trait in enumerate(trait_names):
        print(f"   - {trait}: MAE={mae_per_trait[i]:.4f} | R²={r2[i]:.4f}")
